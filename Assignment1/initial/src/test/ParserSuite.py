...
class ParserSuite(unittest.TestCase):
    def test_a(self):
        input = """
            var BaoLong <- true and "true" or 1 
            var BaoLong <- 1 and 2 and 3 or 4 or 4
            var BaoLong <- 1 + 2 - 2 + 3 and 3
            var BaoLong <- 1 / 2 * 3 % 4
            var BaoLong <- 1 / 2 / 2 * 3 % 4
        """
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 200)) 
    ...
    def test_declared(self): # test case 201 -> 220
        ...
        input = """ 
            func main()
            ## BaoLong
            func main() func main(dynamic a) ## BaoLong
        """
    ...
    def test_Expression(self):
        ...
        input = """ var BaoLong <- "Vo" ... "Tien" 
"""
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 211))
        
        input = """ var BaoLong <- "Vo" ... 1 ... "Tien" 
        """
        ...
        input = """ 
            var BaoLong <- true > "true" 
            var BaoLong <- true >= "true"
            var BaoLong <- true = "true"
            var BaoLong <- true == "true"
            var BaoLong <- true < "true"
            var BaoLong <- true <= "true"
            var BaoLong <- true >= "true" ... 1 > 2
        """
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 213))
        
        input = """ var BaoLong <- true > x >= z 
        """
        ...
        input = """var BaoLong <- true >= "true" and 1 > 2
        """
        ...
        input = """ 
            var BaoLong <- -1 * not 1
            var BaoLong <- not not not----C
        """
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 217)) 
        
        input = """var BaoLong <- - not 1
        """
        ...
        input = """ 
            var BaoLong <- a[1] + 1
            var BaoLong <- array[1,1+2][1][2,3]
            var BaoLong <- array[1,(1)...2,array[ar[(1*2) and 1]],array[2]]
            var BaoLong <- a[1] + fun()[1,fun()] 
            var BaoLong <- 1[1]
        """
        ...
        input = """ 
            var BaoLong <- a()
            var BaoLong <- a(1,2)
            var BaoLong <- a(x,array[2])[2]
            var BaoLong <- a(z,k[3] ... 2)[1,2]
        """
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 221))    
        ...
        input = """ 
            var BaoLong <- a() + ++1 / 2 *3 <= 3 ... "v" >= 2
            var BaoLong <- a(1,2)[1,2,3 ... 2] + false + true
            var BaoLong <- a(z,k[2,3,"2"] ... 2)[true]
            var BaoLong <- (a ... 3) ... b and (a >= b) < b[1, b[1]]
            var BaoLong <-  ["tr", 2, 3, 4, 5] + [[1, 2 + 2 * 2 / 3, 3], [4, 5, 6]]
            var BaoLong <- a(x,array[2])[2,3+2,true,false]
        """
        expect = "successful"
        self.assertTrue(TestParser.test(input, expect, 223))  

        input = """var BaoLong <- a[1]()
        """
